...
def process_folder(folder_path, output_path, folder_num, substance_name):
    # 결과를 저장할 리스트 초기화
    ...
    plt.title(f'Maximum Air Concentration of {substance_name} by Distance Range Over Time')
    plt.legend()
    ...
    # 그래프 저장
    output_file = os.path.join(output_path, f'{folder_num}_concentration_time_series_{substance_name}.png')
...
for i in range(26, 42):
    ...
    folder_path = os.path.join(base_input_path, folder_name, '1minute_interval', 'Air')
    ...
    if os.path.exists(folder_path):
        ...
        process_folder(folder_path, output_path, i, substance_name)
    else:
